...
export async function POST(request: Request) {
  try {
    ...
    // 4. Inicialização sob demanda dos Clientes
    ...
    // 5. Provisionamento Administrativo no Supabase (Com Tratamento de Exceção)
    ...
    // A MÁGICA ACONTECE AQUI: Tratamento Inteligente de Erro
    if (authError) {
      if (authError.message.includes('already been registered')) {
        console.log(`[INFO] Usuário ${customerEmail} já existe no Auth. Seguindo fluxo...`);
      } else {
    ...
    // Ignora erro de duplicidade na tabela clients (se a coluna contact_email for UNIQUE)
    if (dbError && dbError.code !== '23505') { 
      throw new Error(`Falha ao inserir na tabela clients: ${dbError.message}`);
    }
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      to: customerEmail,
      subject: '🚀 [Sync Projetos] Seu acesso ao Cofre + Mentoria foi liberado!',
      html: `
        <div style="font-family: sans-serif; color: #0f172a; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 10px;">
          ...
          <div style="background-color: #f8fafc; padding: 15px; border-radius: 8px; margin: 20px 0;">
            ...
            <p style="margin: 0 0 10px 0;"><strong>Página de Login:</strong> <a href="https://syncprojetossaas.netlify.app/login" style="color: #2563eb; font-weight: bold;">https://syncprojetos.com/login</a></p>
          ...
          <p style="font-size: 12px; color: #64748b;"><em>Nota: Se você já era assinante, sua senha antiga permanece ativa e você pode ignorar a provisória.</em></p>
